...
const Search = () => {
	...
	return (
		<div className="item search" ref={searchRef} tabIndex="4">
			<input
				type="text"
				className={`query-input ${querySuggestions.length
			...
			<div
				className={`setEngine ${engineListVisible ? "removeBottomBorder" : ""
				...
				<div className="query-suggestions">
					{querySuggestions.map((suggestion, idx) => (
						<div
							...
							className={`suggestion ${activeSuggestion == idx
			...
			{engineListVisible ? (
				<div
					className={`engines ${querySuggestions.length ? "engines--transit" : ""
						}`}
					onMouseEnter={showEngines}
					onMouseLeave={hideEngines}
				>
